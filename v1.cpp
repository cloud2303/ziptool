...
bool should_ignore(const fs::path &root_path, const fs::path &current_path,
                   const std::set<fs::path> &ignore_list) {
...
std::set<fs::path>
make_ignore_set(const fs::path &root_path,
...
int compress(const fs::path &zip_path, const fs::path &root_path,
             const std::set<fs::path> &ignore_set, bool windows_style,
             const std::vector<std::pair<fs::path, std::string>> &extra_files,
  ...
  for (fs::recursive_directory_iterator
    ...
    if (entry.is_directory() &&
        should_ignore(root_path, entry.path(), ignore_set)) {
  ...
  for (fs::recursive_directory_iterator
    ...
    if (entry.is_regular_file()) {
      ...
      std::string entry_path =
          windows_style ? wrapper_folder + relative_path.generic_string()
      ...
      processed++;
      if (total_files > 0 &&
          (processed % 50 == 0 || processed == total_files)) {
...
int main(const int argc, char *argv[]) {
  ...
  zip_sub->add_option("-f,--filename", zip_name, "输出的文件名")
      ->default_str("output.zip");
  zip_sub
      ->add_option("-d,--dir", dir_arg, "要压缩的文件路径,当前目录下相对路径")
      ->required();
  zip_sub
      ->add_option("-i,--ignore", ignores,
                   "忽略的相对路径（可多次传入或用逗号分隔）")
      ->delimiter(',');
  zip_sub
      ->add_option("-e,--extra", extra_inputs,
                   "额外压缩的文件路径（可多次传入或用逗号分隔）")
      ->delimiter(',');
  zip_sub->add_flag("-w,--windows-style", windows_style,
                    "Windows压缩风格，套一层与压缩包同名的文件夹");
  ...
  rename_zip
      ->add_option("-d,--dir", rz_dir_arg,
                   "要重命名并压缩的文件夹，相对当前目录")
      ->required();
  rename_zip
      ->add_option("-n,--new-name", rz_new_name,
                   "重命名后的新文件夹名（不含路径）")
      ->required();
  rename_zip->add_flag("-w,--windows-style", rz_windows_style,
                       "Windows压缩风格，套一层与压缩包同名的文件夹");
  ...
  if (rename_zip->parsed()) {
    ...
    const fs::path original_path =
        fs::weakly_canonical(fs::current_path() / rz_dir_arg);
    ...
    if (rz_new_name.empty() ||
        rz_new_name.find_first_of("\\/:") != std::string::npos) {
      std::cerr << "新名称非法（不得包含路径分隔符或保留字符）: " << rz_new_name
                << std::endl;
      return -1;
    }
    ...
    auto restore = [&]() {
      ...
      if (rec) {
        std::cerr << "警告: 无法恢复原始文件夹名称，请手动将 '"
                  << renamed_path.string() << "' 改回 '"
                  << original_path.string() << "'。错误: " << rec.message()
                  << std::endl;
      }
    };
    ...
    const int processed = compress(
        zip_path, renamed_path, ignore_set, rz_windows_style, extra_files,
        [&](const int percent) { bar.set_progress(percent); });
    bar.mark_as_completed();
    // 恢复原名
    restore();

    std::cout << "压缩完成，共处理" << processed << "个文件,输出路径为"
              << zip_path.string() << std::endl;
    return 0;
  }
  ...
  if (zip_sub->parsed()) {
    const fs::path root_path =
        fs::weakly_canonical(fs::current_path() / dir_arg);
    ...
    for (const auto &extra : extra_inputs) {
      ...
      const fs::path absolute_path =
          fs::weakly_canonical(fs::current_path() / input_path);
      ...
      fs::path relative_path =
          fs::relative(absolute_path, fs::current_path(), ec);
    ...
    const int processed =
        compress(zip_path, root_path, ignore_set, windows_style, extra_files,
                 [&](const int percent) { bar.set_progress(percent); });
    bar.mark_as_completed();
    std::cout << "压缩完成，共处理" << processed << "个文件,输出路径为"
              << zip_path.string() << std::endl;
